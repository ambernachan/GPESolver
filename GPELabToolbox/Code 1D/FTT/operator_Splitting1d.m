...
function [Phi] = operator_Splitting1d(Phi, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    % FOR each component where the stochastic dispersion is non null
    for m = FFTPhysics1D.StochasticDispersion_function_Index{n}
        MatcDisp{n,m} = MatcDisp{n,m} + FFTPhysics1D.StochasticDispersion{n,m}; % Computing and adding the potential operator
    end
end
