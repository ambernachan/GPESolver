...
function MakeVideo1d(Method,Geometry1D,Outputs,varargin)
    ...
    Analyse_Var.addOptional('labels', labels, @(x) iscell(x)); % Optional input xlabel, ylabel
    ...
    % Setting inputs
    xaxislabel = Analyse_Var.Results.labels{1}; %Storing the 'xaxislabel' input
    yaxislabel = Analyse_Var.Results.labels{2}; %Storing the 'yaxislabel' input
    ...
    if (MultiVideo == 0)
        ...
        elseif (iscell(Function) == 0)
            ...
            for n = 1:Method.Ncomponents
                ...
                for m = 1:Outputs.Iterations
                   plot(Geometry1D.X,Function(Outputs.Solution{m}{n},Geometry1D.X))
                   xlabel(xaxislabel)
                   ylabel(yaxislabel)
                   writeVideo(vidObj, getframe);
                end
