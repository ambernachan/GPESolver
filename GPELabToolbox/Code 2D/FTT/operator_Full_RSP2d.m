...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = GPE_Phi + 1/Method.Deltat*Phi{n} ...
        + ifft2((-FFTPhysics2D.Delta*(FFTOperators2D.Dx+FFTOperators2D.Dy)/2 + FFTPhysics2D.Dispersion{n,n}/2 + FFTPhysics2D.StochasticDispersion{n,n}/2).*fft2(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
