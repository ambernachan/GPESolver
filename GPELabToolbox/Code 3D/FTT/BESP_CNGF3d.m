...
while (Method.EvolutionCriterion > Method.Stop_crit{2}*Method.Deltat) && (Method.Iterations<Method.Max_iter)
    ...
    Global_L2norm = 0; % Initialization of the global l2 norm
    % FOR each component
    for n = 1:Method.Ncomponents
        FFTPhi{n} = FFTPhi{n}/L2_norm3d(FFTPhi{n},FFTGeometry3D); % Normalization of each wave function
...
Phi_final{n}(FFTGeometry3D.Ny+1,1:FFTGeometry3D.Nx,1:FFTGeometry3D.Nz) = FFTPhi{n}(1,:,:);
Phi_final{n}(1:FFTGeometry3D.Ny,FFTGeometry3D.Nx+1,1:FFTGeometry3D.Nz) = FFTPhi{n}(:,1,:);
Phi_final{n}(1:FFTGeometry3D.Ny,1:FFTGeometry3D.Nx,FFTGeometry3D.Nz+1) = FFTPhi{n}(:,:,1);
Phi_final{n}(FFTGeometry3D.Ny+1,FFTGeometry3D.Nx+1,FFTGeometry3D.Nz+1) = FFTPhi{n}(1,1,1);
end