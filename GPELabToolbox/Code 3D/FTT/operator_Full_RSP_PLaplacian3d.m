...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = Phi{n} + ifftn((1/Method.Deltat-FFTPhysics3D.Delta*(FFTOperators3D.Dx+FFTOperators3D.Dy+FFTOperators3D.Dz)/2 + FFTPhysics3D.Dispersion{n,n}/2 + FFTPhysics3D.StochasticDispersion{n,n}/2).^(-1).*fftn(GPE_Phi)); % Computing the IFFT of the wave function with the operators applied then apply the Laplace preconditioner
