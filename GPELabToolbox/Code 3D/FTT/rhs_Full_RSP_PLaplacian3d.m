...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = GPE_Phi + 1/Method.Deltat*Phi{n} ...
        + ifftn((FFTPhysics3D.Delta*(FFTOperators3D.Dx+FFTOperators3D.Dy+FFTOperators3D.Dz)/2 - FFTPhysics3D.Dispersion{n,n}/2 - FFTPhysics3D.StochasticDispersion{n,n}/2).*fftn(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
    GPE_Phi = ifftn((1/Method.Deltat-FFTPhysics3D.Delta/2*(FFTOperators3D.Dx+FFTOperators3D.Dy+FFTOperators3D.Dz) + FFTPhysics3D.Dispersion{n,n}/2 + FFTPhysics3D.StochasticDispersion{n,n}/2).^(-1).*fftn(GPE_Phi)); % Applying the Laplace preconditioner
