...
exponent = exp(-(gamma_x*(Geometry3D.X-X0).^2+gamma_y*(Geometry3D.Y-Y0).^2+gamma_z*(Geometry3D.Z-Z0).^2)/2);
phi_0 = (gamma_x*gamma_y*gamma_z)^(1/4)*((1-sum(Physics3D.Omega)).*exponent/sqrt(pi)) ...
        + Physics3D.Omega(3).*((gamma_x*(Geometry3D.X-X0)+1i*gamma_y*(Geometry3D.Y-Y0)).*exponent/sqrt(pi))...
        + Physics3D.Omega(1).*((gamma_y*(Geometry3D.Y-Y0)+1i*gamma_z*(Geometry3D.Z-Z0)).*exponent/sqrt(pi))...
        + Physics3D.Omega(2).*((gamma_z*(Geometry3D.Z-Z0)+1i*gamma_x*(Geometry3D.X-X0)).*exponent/sqrt(pi)); % Computing the centered gaussian
