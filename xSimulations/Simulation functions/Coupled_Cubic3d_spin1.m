...
%     CoupledSpin1Nonlin{1,2} = @(Phi,X,Y,Z) Betas * Phi{3}' .* Phi{2};
%     CoupledSpin1Nonlin{2,1} = @(Phi,X,Y,Z) Betas * Phi{2}' .* Phi{3};
%     CoupledSpin1Nonlin{2,3} = @(Phi,X,Y,Z) Betas * Phi{2}' .* Phi{1};
%     CoupledSpin1Nonlin{3,2} = @(Phi,X,Y,Z) Betas * Phi{1}' .* Phi{2};
    CoupledSpin1Nonlin{1,2} = @(Phi,X,Y,Z) Betas * conj(Phi{3}) .* Phi{2};
    CoupledSpin1Nonlin{2,1} = @(Phi,X,Y,Z) Betas * conj(Phi{2}) .* Phi{3};
    CoupledSpin1Nonlin{2,3} = @(Phi,X,Y,Z) Betas * conj(Phi{2}) .* Phi{1};
    CoupledSpin1Nonlin{3,2} = @(Phi,X,Y,Z) Betas * conj(Phi{1}) .* Phi{2};
    
    CoupledSpin1Nonlin{3,1} = @(Phi,X,Y,Z) 0;
    CoupledSpin1Nonlin{1,3} = @(Phi,X,Y,Z) 0;
