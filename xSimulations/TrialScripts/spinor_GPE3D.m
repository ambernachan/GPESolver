...
function [] = spinor_GPE3D(info)
    global hbar amu abohr
    ...
    trapfreq = 100 * 2*pi; % Trap strength in Hz (symmetric for now)
    atom_mass = 22.989769; % Atom mass in amu
    
    atom_mass = atom_mass * amu;
    aho = sqrt(hbar / (atom_mass * trapfreq));
    chin = N*(2*a2+a0)*abohr/(3*aho);
    chis = N*(a2-a0)*abohr/(3*aho);
...
%         Coupled_CubicEnergy3d_spin1(Betan,Betas)); % cubic nonlinearity with off-diagonal coupling
    functiehier = Coupled_Cubic3d_spin1(Betan,Betas);
    Physics3D = Nonlinearity_Var3d(Method, Physics3D, functiehier); % cubic nonlinearity with off-diagonal coupling
