...
if nargin > 5
    ...
    if ~or(strcmp(difference_type, 'minus'),strcmp(difference_type, 'ratio'))
        difference_type = input('Give difference type "minus" or "ratio"');
    end
end
...
for n = 1:Method.Ncomponents
    ...
    % For setting the limits of the colorbar
    bottom = min(min(min(phi1_sq{n})),min(min(phi2_sq{n})));
    top  = max(max(max(phi1_sq{n})),max(max(phi2_sq{n})));
